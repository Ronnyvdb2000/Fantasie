...
def run_live_engine():
    ...
    for ex_name, tlist in exchange_tickers.items():
        ...
        if signalen or exits:
            ...
            nu = today_str()
            bericht_delen = [
                f"🚀 *DUAL MOMENTUM — {ex_name}*",
                f"_{nu} | Top 20% omzet × RS → 52w high | {len(signalen)} kandidaten_",
            ]
            if exits:
                bericht_delen.append("─────────────────────────────")
                bericht_delen.append(f"🔴 *EXIT SIGNALEN ({len(exits)}):*")
                for e in exits:
                    bericht_delen.append(f"• `{e['ticker']}` {e['reason']} | €{e['price']:.2f}")
            if signalen:
                bericht_delen += ["─────────────────────────────", f"🏆 *TOP 2:*"]
                for s in top2:
                    bericht_delen.append(f"• `{s.ticker}` {_score_bar(s.score)} €{s.price:.2f} [Grafiek](https://finance.yahoo.com/quote/{s.ticker})\n  Omzet+{s.rev_growth:.1f}% RS:{s.rs:.0f} | {s.pct_from_high:.1f}% onder high\n" + sizing_tekst(s.ticker, s.price, s.stop, s.ma50, portfolio_waarde))
                bericht_delen += ["─────────────────────────────", f"*{lbl}:*"]
                for s in [x for x in toon if x not in top2]:
                    bericht_delen.append(f"• `{s.ticker}` {_score_bar(s.score)} €{s.price:.2f} | omzet+{s.rev_growth:.1f}% RS:{s.rs:.0f}")
            bericht_delen.append(f"⚙️ _Top {DM_CFG['top_pct']:.0f}% omzet×RS | Exit<MA{DM_CFG['ma_exit']} | Risico 5%_")
            bericht = "\n\n".join(bericht_delen)
