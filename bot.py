...
def analyseer_aandeel(ticker):
    ...
    # De .squeeze() zorgt ervoor dat we altijd een Series krijgen, wat ValueErrors voorkomt
    ...
    # Pak specifieke waarden met .iloc (Vandaag = -1, Gisteren = -2)
    huidige_koers = close_prices.iloc[-1]
    vorige_koers = close_prices.iloc[-2]
    
    sma50_nu = sma50.iloc[-1]
    sma200_nu = sma200.iloc[-1]
    sma50_gisteren = sma50.iloc[-2]
    sma200_gisteren = sma200.iloc[-2]
    
    huidig_volume = volumes.iloc[-1]
    gem_volume = avg_volume.iloc[-1]
