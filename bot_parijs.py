...
def bereken_rsi(data, window=14):
    ...
    rs = ema_up / (ema_down + 1e-10) # Kleine constante om deling door nul te voorkomen
    return 100 - (100 / (1 + rs))
...
def bereken_bt(df, inzet, s, t):
    VASTE_KOST = 7.50
    BEURSTAKS = 0.0035
    FRANSE_FTT = 0.0040 # 0.4% taks bij aankoop
    ...
    for i in range(1, len(data)):
        ...
        if not pos and f_nu > s_nu and f_oud <= s_oud:
            # Aankoopkosten inclusief Franse FTT
            kosten = VASTE_KOST + (inzet * BEURSTAKS) + (inzet * FRANSE_FTT)
            k, pos = prijs, True
            saldo -= kosten
        elif pos and f_nu < s_nu and f_oud >= s_oud:
            # Verkoopkosten
            bruto = inzet * (prijs / k)
            kosten = VASTE_KOST + (bruto * BEURSTAKS)
            saldo += (bruto - inzet) - kosten
            pos = False
    return saldo

def main():
    stuur_telegram("🇫🇷 *PARIJS: SCAN INCLUSIEF RSI & FRANSE TAX*")
    ...
    rapport = f"🇫🇷 *RENDEMENTSRAPPORT PARIJS*\n----------------------------------\n"
