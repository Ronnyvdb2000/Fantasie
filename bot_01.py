...
def inline_backtest(df_ex: pd.DataFrame, inzet: float = 2500.0) -> Dict[str, dict]:
    """
    Inline simulatie identiek aan bot_00ULT:
    - Simuleert trades over de volledige historiek (5 jaar)
    - Per strategie: MA-crossover instap + trailing stop/MA-exit
    - Startkapitaal EUR 100.000 (puur voor weergave rendement)
    - Retourneert dict met eindwaarde en aantal trades per strategie
    """
    START_SIM  = 100_000.0
    KOSTEN     = 15.0 + (inzet * 0.0035)

    STRATS = [
        # (key, naam,         fast_per, slow_per, use_ema200, is_hyper_adx)
        ("T",  "Traag 50/200",   50,  200, True),
        ("S",  "Snel 20/50",     20,   50, True),
        ("HT", "Hyper Trend",     9,   21, True),
        ("HS", "Hyper Scalp",     9,   21, False),
    ]

    resultaten: Dict[str, dict] = {}

    tickers = df_ex["Ticker"].unique()

    # Bereken per strategie over alle tickers
    for skey, snaam, fast_p, slow_p, use_ema200 in STRATS:
        pnl_totaal = 0.0
        n_trades   = 0

        for ticker in tickers:
            df_t = df_ex[df_ex["Ticker"] == ticker].copy().reset_index(drop=True)
            if len(df_t) < 250:
                continue
            ...
            # Fast/slow lijn
            if fast_p >= 20:
                f_line = close.rolling(fast_p).mean()
            else:
                f_line = close.ewm(span=fast_p, adjust=False).mean()

            if slow_p >= 50:
                s_line = close.rolling(slow_p).mean()
            else:
                s_line = close.ewm(span=slow_p, adjust=False).mean()
            ...
            plus_di  = 100 * plus_dm.ewm(alpha=1/14, adjust=False).mean() / (atr.ewm(alpha=1/14, adjust=False).mean() + 1e-10)
            minus_di = 100 * minus_dm.ewm(alpha=1/14, adjust=False).mean() / (atr.ewm(alpha=1/14, adjust=False).mean() + 1e-10)
            dx       = (plus_di - minus_di).abs() / (plus_di + minus_di + 1e-10) * 100
            adx      = dx.ewm(alpha=1/14, adjust=False).mean()

            # Simuleer trades (start na 200 candles voor stabilisatie indicatoren)
            pnl  = 0.0
            pos  = False
            ins  = 0.0
            hi   = 0.0

            for i in range(201, len(df_t)):
                cp      = close.iloc[i]
                f_cur   = f_line.iloc[i]
                f_prev  = f_line.iloc[i-1]
                s_cur   = s_line.iloc[i]
                s_prev  = s_line.iloc[i-1]
                adx_cur = adx.iloc[i]
                vol_cur = vol.iloc[i]
                vma_cur = vol_ma.iloc[i]
                atr_cur = atr.iloc[i]
                e200    = ema200.iloc[i]

                if not pos:
                    # Instap: crossover + ADX > 15 + volume + optioneel EMA200
                    if (f_cur > s_cur and f_prev <= s_prev
                            and adx_cur > 15
                            and vol_cur > (vma_cur * 0.6)
                            and (not use_ema200 or cp > e200)):
                        ins = cp
                        hi  = cp
                        pos = True
                        pnl -= KOSTEN
                    ...
                    hi = max(hi, cp)
                    # Exit: trailing stop (2x ATR) OF MA-crossover omlaag
                    if cp < (hi - 2 * atr_cur) or f_cur < s_cur:
                        pnl += (inzet * (cp / ins) - inzet) - KOSTEN
                        pos  = False

            # Open positie sluiten op laatste koers
            if pos:
                cp   = close.iloc[-1]
                pnl += (inzet * (cp / ins) - inzet) - KOSTEN
        ...
        resultaten[skey] = {
            "naam":      snaam,
            "eindwaarde": START_SIM + pnl_totaal,
            "n_trades":  n_trades,
            "pnl":       pnl_totaal,
        }
...
def format_signals_per_exchange(
    ...
    portfolio:     LivePortfolio,
    df_ex:         Optional[pd.DataFrame] = None,
) -> Tuple[str, str]:
    """
    ...
    # Inline backtest uitvoeren op de exchange-specifieke data
    try:
        bt = inline_backtest(df_ex, inzet=BASE_POSITION_SIZE) if df_ex is not None and not df_ex.empty else {}
    except Exception as _bt_err:
        print(f"[WARN] Backtest fout: {_bt_err}")
        bt = {}

    def fmt_bt(key: str) -> str:
        if key not in bt:
            return "n/a"
        d = bt[key]
        pijl = "📈" if d["pnl"] >= 0 else "📉"
        return f"{pijl} EUR{d['eindwaarde']:,.0f} ({d['n_trades']} trades)"
    ...
    deel1_lines = [
        ...
        "----------------------------------",
        "",
        "📊 *BACKTEST 5J (per strategie, inzet EUR2500):*",
        f"🐢 Traag 50/200 : {fmt_bt('T')}",
        f"⚡ Snel 20/50   : {fmt_bt('S')}",
        f"🚀 Hyper Trend  : {fmt_bt('HT')}",
        f"🔥 Hyper Scalp  : {fmt_bt('HS')}",
        "",
...
def run_live_engine():
    ...
    for ex_name, tlist in exchange_tickers.items():
        ...
        deel1, deel2 = format_signals_per_exchange(ex_name, filtered_buys, exit_ex, portfolio, df_ex)
