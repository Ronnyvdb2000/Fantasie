#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
bouw_generieke_technicals.py  —  GEDEELDE TECHNISCHE INDICATOREN  v2.0

DOEL
====
Lost het "n_strategieen=1"-probleem van analyse_parameter_correlatie.py op:
een technische indicator (ATR%, RSI, MA-afstand, volumeratio, 52w-hoogte,
MACD, Bollinger, Stochastic, ADX, relatieve sterkte, volatiliteit) hangt
niet af van WELKE strategie een aandeel koos, enkel van de koers-
/volumegeschiedenis op die datum. Door deze centraal en strategie-
onafhankelijk te berekenen -- één rij per (ticker, datum), niet per
(ticker, datum, strategie) -- krijgt ELKE selectie uit ELKE bot dezelfde
kolommen ingevuld, i.p.v. enkel de rijen van de ene bot die toevallig zijn
eigen ATR berekent.

GEEN LOOK-AHEAD BIAS: enkel indicatoren die uit koers-/volumedata TOT EN MET
`datum` berekenbaar zijn. Fundamentele data (ROE, P/E, FCF-yield, Piotroski,
sector) wordt hier BEWUST NIET opgenomen -- yfinance geeft daarvan enkel de
HUIDIGE momentopname, geen historisch puntmoment, dus retroactief invullen
zou toekomstige informatie in het verleden plaatsen. Die horen in de
aparte, NIET-backfillbare generieke_fundamentals-tabel (enkel nieuwe
selecties vanaf vandaag) -- zie bouw_generieke_fundamentals.py.

v2.0 (2026-09-26): toegevoegd t.o.v. v1.0 -- MACD(+signaal+histogram),
Bollinger Bands (breedte + %B), Stochastic %K/%D, ADX14, relatieve sterkte
t.o.v. REFERENTIE_INDEX over 20 handelsdagen, historische volatiliteit
(20d/60d, geannualiseerd), dagen sinds 52w-low, vol_ratio_50d (naast de
bestaande 20d). Alle nieuwe indicatoren zijn, net als de bestaande, puur
uit koers-/volumedata tot en met `datum` berekend -- geen look-ahead.
LET OP: bestaande (ticker, datum)-rijen worden NIET automatisch met de
nieuwe kolommen herberekend -- haal_openstaande_paren() filtert enkel op
"rij bestaat nog niet". Enkel écht nieuwe paren krijgen de nieuwe
kolommen vanaf nu ingevuld. Voor een backfill van bestaande rijen is een
apart, eenmalig script nodig (analoog aan backfill_rank_kolommen.py).

INCREMENTEEL, GEEN WACHTTIJD NODIG (in tegenstelling tot forward_returns):
een technische indicator op datum X is METEEN berekenbaar zodra X voorbij
is, geen 5/10/20 handelsdagen wachten zoals bij forward-rendement. Elke
run vult dus zowel de historische achterstand als de nieuwste selecties
van gisteren/vandaag aan.

Zelfde patronen als bouw_forward_returns.py: bulk yfinance-download per
ticker (dekt alle openstaande datums van die ticker in 1 call), upsert
(ON CONFLICT DO UPDATE), MAX_TICKERS_PER_RUN als veiligheidslimiet,
prioriteit op oudste openstaande datum i.p.v. alfabetisch (zelfde
fairness-fix als daar). De referentie-index wordt ÉÉN keer per run
gedownload (niet per ticker) en hergebruikt voor de relatieve-sterkte-
berekening van elke ticker.

GEBRUIK
=======
  python bouw_generieke_technicals.py build

Env vars: SUPABASE_DB_URL (verplicht), TELEGRAM_TOKEN/TELEGRAM_CHAT_ID
(optioneel), MAX_TICKERS_PER_RUN (default 400), REFERENTIE_INDEX
(default "^GSPC" -- S&P500; pas aan indien elders in het project een
andere index als marktreferentie gebruikt wordt, bv. voor markt_ret_5d)
"""

import os
import sys
import math
import time
from datetime import datetime, timezone
from typing import Dict, List, Optional

import numpy as np
import psycopg2
import psycopg2.extras
import yfinance as yf
import pandas as pd
import requests

SUPABASE_DB_URL = os.environ.get("SUPABASE_DB_URL")
TELEGRAM_TOKEN = os.environ.get("TELEGRAM_TOKEN", "")
TELEGRAM_CHAT_ID = os.environ.get("TELEGRAM_CHAT_ID", "")
MAX_TICKERS_PER_RUN = int(os.environ.get("MAX_TICKERS_PER_RUN", "400"))
# LET OP: nog niet bevestigd of dit overeenkomt met de index die elders in
# het project (bv. markt_ret_5d in selecties) al gebruikt wordt -- pas aan
# indien dat een andere index is, voor consistentie tussen beide.
REFERENTIE_INDEX = os.environ.get("REFERENTIE_INDEX", "^GSPC")

RSI_PERIOD = 14
ATR_PERIOD = 14
ADX_PERIOD = 14
STOCH_PERIOD = 14
BB_PERIOD = 20
MA_KORT = 50
MA_LANG = 200
REL_STERKTE_PERIODE = 20
HV_KORT = 20
HV_LANG = 60
LOW52W_WINDOW = 252
# genoeg extra historiek vóór de vroegste benodigde datum om MA200/52w-hoogte
# op DIE datum al zinvol te kunnen berekenen (geen look-ahead: enkel data
# tot en met de datum zelf wordt gebruikt, dit is puur de opstartbuffer)
LOOKBACK_BUFFER_DAGEN = 380


def vandaag() -> str:
    return datetime.now(timezone.utc).strftime("%Y-%m-%d")


def send_telegram(tekst: str) -> None:
    if not TELEGRAM_TOKEN or not TELEGRAM_CHAT_ID:
        print(tekst)
        return
    try:
        requests.post(
            f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendMessage",
            json={"chat_id": TELEGRAM_CHAT_ID, "text": tekst, "parse_mode": "Markdown"},
            timeout=10,
        )
    except Exception as e:
        print(f"Telegram fout: {e}")


# --------------------------------------------------------------------------
# Wilder-smoothing, zelfde conventie als bot_00mr.py / bot_01volhunter.py
# --------------------------------------------------------------------------
def _wilder(series: pd.Series, period: int) -> pd.Series:
    result = pd.Series(index=series.index, dtype=float)
    valid = series.dropna()
    if len(valid) < period:
        return result
    result[valid.index[period - 1]] = valid.iloc[:period].mean()
    for i in range(period, len(valid)):
        result[valid.index[i]] = (
            result[valid.index[i - 1]] * (period - 1) / period
            + valid.iloc[i] / period
        )
    return result


# --------------------------------------------------------------------------
# Referentie-index: één keer per run gedownload, hergebruikt voor elke ticker
# --------------------------------------------------------------------------
def download_index_returns(start: str) -> Optional[pd.Series]:
    """Geeft het REL_STERKTE_PERIODE-daags rendement (%) van de referentie-
    index terug, per handelsdag -- gebruikt om elke ticker's eigen
    20-daags rendement tegen af te zetten (relatieve sterkte)."""
    try:
        hist = yf.Ticker(REFERENTIE_INDEX).history(start=start, auto_adjust=True)
    except Exception as e:
        print(f"  [WARN] Referentie-index {REFERENTIE_INDEX}: download mislukt ({e})")
        return None
    if hist is None or hist.empty or "Close" not in hist.columns:
        print(f"  [WARN] Referentie-index {REFERENTIE_INDEX}: geen koersdata")
        return None
    hist.index = pd.to_datetime(hist.index).tz_localize(None)
    close = hist["Close"]
    return (close / close.shift(REL_STERKTE_PERIODE) - 1) * 100


def bereken_indicatoren(hist: pd.DataFrame, index_ret: Optional[pd.Series]) -> pd.DataFrame:
    g = hist.copy()
    close, high, low, volume = g["Close"], g["High"], g["Low"], g["Volume"]

    g["IBS"] = (close - low) / (high - low + 1e-9)

    delta = close.diff()
    gain = delta.clip(lower=0)
    loss = (-delta).clip(lower=0)
    rs = _wilder(gain, RSI_PERIOD) / (_wilder(loss, RSI_PERIOD) + 1e-9)
    g["RSI14"] = 100 - (100 / (1 + rs))

    hl = high - low
    hcp = (high - close.shift()).abs()
    lcp = (low - close.shift()).abs()
    tr = pd.concat([hl, hcp, lcp], axis=1).max(axis=1)
    g["ATR14"] = _wilder(tr, ATR_PERIOD)
    g["ATR14_PCT"] = g["ATR14"] / close * 100

    g["MA50"] = close.rolling(MA_KORT).mean()
    g["MA200"] = close.rolling(MA_LANG).mean()
    g["PCT_FROM_MA50"] = (close / g["MA50"] - 1) * 100
    g["PCT_FROM_MA200"] = (close / g["MA200"] - 1) * 100

    basis_vol_20 = volume.rolling(20).mean()
    g["VOL_RATIO_20D"] = volume / basis_vol_20
    basis_vol_50 = volume.rolling(50).mean()
    g["VOL_RATIO_50D"] = volume / basis_vol_50

    g["HIGH52W"] = high.rolling(LOW52W_WINDOW, min_periods=50).max()
    g["PCT_FROM_HIGH52W"] = (close / g["HIGH52W"] - 1) * 100

    # --- MACD (12/26/9) ---
    ema12 = close.ewm(span=12, adjust=False).mean()
    ema26 = close.ewm(span=26, adjust=False).mean()
    g["MACD"] = ema12 - ema26
    g["MACD_SIGNAAL"] = g["MACD"].ewm(span=9, adjust=False).mean()
    g["MACD_HIST"] = g["MACD"] - g["MACD_SIGNAAL"]

    # --- Bollinger Bands (20d, 2 std) ---
    ma20 = close.rolling(BB_PERIOD).mean()
    std20 = close.rolling(BB_PERIOD).std()
    bb_upper = ma20 + 2 * std20
    bb_lower = ma20 - 2 * std20
    g["BB_BREEDTE"] = (bb_upper - bb_lower) / ma20 * 100
    g["BB_PERCENT_B"] = (close - bb_lower) / (bb_upper - bb_lower + 1e-9)

    # --- Stochastic %K/%D (14/3) ---
    low14 = low.rolling(STOCH_PERIOD).min()
    high14 = high.rolling(STOCH_PERIOD).max()
    g["STOCH_K"] = (close - low14) / (high14 - low14 + 1e-9) * 100
    g["STOCH_D"] = g["STOCH_K"].rolling(3).mean()

    # --- ADX14 (trendsterkte, Wilder) ---
    up_move = high.diff()
    down_move = -low.diff()
    plus_dm = pd.Series(
        np.where((up_move > down_move) & (up_move > 0), up_move, 0.0), index=close.index
    )
    minus_dm = pd.Series(
        np.where((down_move > up_move) & (down_move > 0), down_move, 0.0), index=close.index
    )
    plus_di = 100 * _wilder(plus_dm, ADX_PERIOD) / (g["ATR14"] + 1e-9)
    minus_di = 100 * _wilder(minus_dm, ADX_PERIOD) / (g["ATR14"] + 1e-9)
    dx = (plus_di - minus_di).abs() / (plus_di + minus_di + 1e-9) * 100
    g["ADX14"] = _wilder(dx, ADX_PERIOD)

    # --- Historische volatiliteit (log-rendementen, geannualiseerd, %) ---
    log_ret = np.log(close / close.shift(1))
    g["HV20"] = log_ret.rolling(HV_KORT).std() * math.sqrt(252) * 100
    g["HV60"] = log_ret.rolling(HV_LANG).std() * math.sqrt(252) * 100

    # --- Dagen sinds 52w-low (spiegelbeeld van bestaande high52w-logica) ---
    def _dagen_sinds_min(venster: np.ndarray) -> float:
        return len(venster) - 1 - int(np.argmin(venster))

    g["DAGEN_SINDS_LOW52W"] = (
        low.rolling(LOW52W_WINDOW, min_periods=50)
        .apply(_dagen_sinds_min, raw=True)
    )

    # --- Relatieve sterkte t.o.v. referentie-index (20d rendement-verschil) ---
    ticker_ret_20d = (close / close.shift(REL_STERKTE_PERIODE) - 1) * 100
    if index_ret is not None:
        index_ret_aligned = index_ret.reindex(g.index, method="ffill")
        g["REL_STERKTE_20D"] = ticker_ret_20d - index_ret_aligned
    else:
        g["REL_STERKTE_20D"] = np.nan

    return g


# --------------------------------------------------------------------------
# Stap 1: welke (ticker, datum)-paren ontbreken nog in generieke_technicals?
# --------------------------------------------------------------------------
def haal_openstaande_paren(conn) -> List[dict]:
    # ticker LIKE '%/%' sluit o.a. bot_01cointegr.py's samengestelde
    # "TICKER_A/TICKER_B"-paarstrings uit -- die kunnen nooit via yfinance
    # opgelost worden en zouden anders élke dag opnieuw (en voor altijd)
    # als "openstaand" blijven terugkomen zonder ooit te slagen.
    query = """
        SELECT DISTINCT s.ticker, s.datum
        FROM selecties s
        LEFT JOIN generieke_technicals g
          ON s.ticker = g.ticker AND s.datum = g.datum
        WHERE g.ticker IS NULL
          AND s.ticker NOT LIKE '%/%'
        ORDER BY s.ticker, s.datum;
    """
    with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
        cur.execute(query)
        return cur.fetchall()


# --------------------------------------------------------------------------
# Stap 2: per ticker het koersverloop ophalen en indicatoren berekenen
# --------------------------------------------------------------------------
def bereken_voor_ticker(ticker: str, datums: List[str], index_ret: Optional[pd.Series]) -> List[dict]:
    vroegste = min(datums)
    start = (pd.Timestamp(vroegste) - pd.Timedelta(days=LOOKBACK_BUFFER_DAGEN)).strftime("%Y-%m-%d")
    try:
        hist = yf.Ticker(ticker).history(start=start, auto_adjust=True)
    except Exception as e:
        print(f"  [WARN] {ticker}: download mislukt ({e})")
        return []

    if hist is None or hist.empty or "Close" not in hist.columns:
        print(f"  [WARN] {ticker}: geen koersdata")
        return []

    hist.index = pd.to_datetime(hist.index).tz_localize(None)
    g = bereken_indicatoren(hist, index_ret)

    resultaten = []
    for datum in datums:
        try:
            doel = pd.Timestamp(datum)
            pos = g.index.searchsorted(doel)
            if pos >= len(g):
                continue
            # exacte handelsdag pakken indien aanwezig, anders de eerstvolgende
            rij = g.iloc[pos]

            def veilig(waarde, is_int: bool = False):
                try:
                    f = float(waarde)
                    if math.isnan(f):
                        return None
                    return int(round(f)) if is_int else round(f, 4)
                except Exception:
                    return None

            resultaten.append({
                "ticker": ticker, "datum": datum,
                "atr14": veilig(rij["ATR14"]), "atr14_pct": veilig(rij["ATR14_PCT"]),
                "rsi14": veilig(rij["RSI14"]), "ibs": veilig(rij["IBS"]),
                "ma50": veilig(rij["MA50"]), "ma200": veilig(rij["MA200"]),
                "pct_from_ma50": veilig(rij["PCT_FROM_MA50"]),
                "pct_from_ma200": veilig(rij["PCT_FROM_MA200"]),
                "vol_ratio_20d": veilig(rij["VOL_RATIO_20D"]),
                "high52w": veilig(rij["HIGH52W"]), "pct_from_high52w": veilig(rij["PCT_FROM_HIGH52W"]),
                "macd": veilig(rij["MACD"]), "macd_signaal": veilig(rij["MACD_SIGNAAL"]),
                "macd_hist": veilig(rij["MACD_HIST"]),
                "bb_breedte": veilig(rij["BB_BREEDTE"]), "bb_percent_b": veilig(rij["BB_PERCENT_B"]),
                "stoch_k": veilig(rij["STOCH_K"]), "stoch_d": veilig(rij["STOCH_D"]),
                "adx14": veilig(rij["ADX14"]),
                "rel_sterkte_20d": veilig(rij["REL_STERKTE_20D"]),
                "hv20": veilig(rij["HV20"]), "hv60": veilig(rij["HV60"]),
                "dagen_sinds_low52w": veilig(rij["DAGEN_SINDS_LOW52W"], is_int=True),
                "vol_ratio_50d": veilig(rij["VOL_RATIO_50D"]),
            })
        except Exception as e:
            print(f"  [WARN] {ticker} {datum}: {e}")
            continue

    return resultaten


# --------------------------------------------------------------------------
# Stap 3: upsert
# --------------------------------------------------------------------------
KOLOMMEN = [
    "ticker", "datum", "atr14", "atr14_pct", "rsi14", "ibs", "ma50", "ma200",
    "pct_from_ma50", "pct_from_ma200", "vol_ratio_20d", "high52w", "pct_from_high52w",
    "macd", "macd_signaal", "macd_hist", "bb_breedte", "bb_percent_b",
    "stoch_k", "stoch_d", "adx14", "rel_sterkte_20d", "hv20", "hv60",
    "dagen_sinds_low52w", "vol_ratio_50d",
]


def upsert_rijen(conn, rijen: List[dict]) -> int:
    if not rijen:
        return 0
    kolom_lijst = ", ".join(KOLOMMEN)
    placeholders = ", ".join(f"%({k})s" for k in KOLOMMEN)
    update_lijst = ", ".join(f"{k} = EXCLUDED.{k}" for k in KOLOMMEN if k not in ("ticker", "datum"))
    update_lijst += ", bijgewerkt_op = now()"

    query = f"""
        INSERT INTO generieke_technicals ({kolom_lijst})
        VALUES ({placeholders})
        ON CONFLICT (ticker, datum)
        DO UPDATE SET {update_lijst};
    """
    with conn.cursor() as cur:
        for rij in rijen:
            cur.execute(query, {k: rij.get(k) for k in KOLOMMEN})
    conn.commit()
    return len(rijen)


# --------------------------------------------------------------------------
# Main
# --------------------------------------------------------------------------
def run_build():
    if not SUPABASE_DB_URL:
        print("FOUT: SUPABASE_DB_URL ontbreekt.", file=sys.stderr)
        sys.exit(1)

    conn = psycopg2.connect(SUPABASE_DB_URL)
    try:
        open_paren = haal_openstaande_paren(conn)
        print(f"{len(open_paren)} (ticker, datum)-paren nog niet aanwezig in generieke_technicals.")

        if not open_paren:
            print("Niets te doen.")
            return

        per_ticker: Dict[str, List[str]] = {}
        for r in open_paren:
            per_ticker.setdefault(r["ticker"], []).append(r["datum"])

        # zelfde fairness-fix als bouw_forward_returns.py: oudste eerst
        oudste_per_ticker = {t: min(datums) for t, datums in per_ticker.items()}
        alle_tickers_gesorteerd = sorted(per_ticker.keys(), key=lambda t: oudste_per_ticker[t])
        tickers = alle_tickers_gesorteerd[:MAX_TICKERS_PER_RUN]
        overgeslagen = len(per_ticker) - len(tickers)
        print(f"{len(tickers)} unieke tickers te verwerken dit run"
              + (f" ({overgeslagen} tickers volgen in een volgend run)" if overgeslagen > 0 else ""))

        # Referentie-index één keer downloaden voor de hele run, i.p.v. per
        # ticker -- dezelfde index-data wordt door elke ticker hergebruikt
        # voor de relatieve-sterkte-berekening.
        vroegste_algemeen = min(oudste_per_ticker[t] for t in tickers)
        index_start = (
            pd.Timestamp(vroegste_algemeen) - pd.Timedelta(days=LOOKBACK_BUFFER_DAGEN)
        ).strftime("%Y-%m-%d")
        print(f"Referentie-index {REFERENTIE_INDEX} downloaden vanaf {index_start}...")
        index_ret = download_index_returns(index_start)
        if index_ret is None:
            print("  [WARN] Referentie-index niet beschikbaar -- rel_sterkte_20d blijft leeg dit run.")

        totaal_bijgewerkt = 0
        for i, ticker in enumerate(tickers, start=1):
            rijen = bereken_voor_ticker(ticker, per_ticker[ticker], index_ret)
            totaal_bijgewerkt += upsert_rijen(conn, rijen)
            if i % 25 == 0 or i == len(tickers):
                print(f"  {i}/{len(tickers)} tickers verwerkt...")
            time.sleep(0.1)

        send_telegram(
            f"📊 *Generieke Technicals — {vandaag()}*\n\n"
            f"{totaal_bijgewerkt} (ticker, datum)-rijen bijgewerkt\n"
            f"{len(tickers)} unieke tickers verwerkt dit run"
            + (f" ({overgeslagen} tickers nog te gaan)" if overgeslagen > 0 else "")
        )
        print(f"\nKlaar. {totaal_bijgewerkt} rijen bijgewerkt.")
    finally:
        conn.close()


if __name__ == "__main__":
    mode = sys.argv[1].lower() if len(sys.argv) > 1 else "build"
    run_build()
