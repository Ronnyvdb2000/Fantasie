...
# Laad omgevingsvariabelen voor Telegram
load_dotenv()
...
def bereken_alles(ticker, inzet, s, t, use_trend_filter=False):
    """
    Core Engine van de MacroTrends Bot:
    Berekent rendement over 1 jaar en genereert signalen incl. ATR en Stop Loss.
    """
    try:
        # Download data (5 jaar nodig voor stabiele EMA200)
        df = yf.download(ticker, period="5y", progress=False, auto_adjust=True)
        if df.empty or len(df) < 260: return 0, None
        
        # Data isolatie voor robuustheid
        ...
        # ATR (Volatiliteit) & ADX (Trendsterkte)
        ...
        # --- DEEL 1: BACKTEST (Laatste 252 handelsdagen) ---
...
def main():
    ...
    ticker_file = 'tickers_02.txt'
    
    if not os.path.exists(ticker_file):
        print(f"Fout: {ticker_file} niet gevonden.")
        return
    ...
    for t in tickers:
        # T=Traag, S=Snel, HT=Hyper Trend, HS=Hyper Scalp
    ...
    rapport = [
        "🌐 *MACROTRENDS STRATEGY RAPPORT*",
        f"_{nu}_",
        "----------------------------------",
        f"🐢 *Traag (50/200 SMA):* €{100000 + res['T']:,.0f}",
        f"⚡ *Snel (20/50 SMA):* €{100000 + res['S']:,.0f}",
        f"🚀 *Hyper Trend (9/21 EMA+200):* €{100000 + res['HT']:,.0f}",
        f"🔥 *Hyper Scalp (9/21 EMA):* €{100000 + res['HS']:,.0f}",
        "",
        ...
        "",
        "⚙️ _Bron: tickers_02.txt | Filters: ADX, Vol, ATR Stop Loss active_"
    ]
    stuur_telegram("\n".join(rapport))
