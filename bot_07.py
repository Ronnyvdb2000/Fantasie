...
def check_live_signaal(df, s, t, ticker):
    ...
    prijs_nu = round(df['Close'].iloc[-1], 2) # Haalt de huidige koers op
    link = f"https://finance.yahoo.com/quote/{ticker}"
    
    # Status labels specifiek voor grondstoffen
    status = "⚠️ OVERVERHIT" if rsi_nu > 70 else "✅ GEZOND"
    if rsi_nu < 30: status = "🔥 EXTREEM ONDERGEWAARDEERD"
    elif rsi_nu < 45: status = "💎 INTERESSANT"
...
def bereken_bt(df, inzet, s, t):
    # Bolero tarieven buitenlandse beurzen (NYSE/TSX/LSE)
    VASTE_KOST = 15.00 
    BEURSTAKS = 0.0035 
...
def main():
    stuur_telegram("🥇🥈🥉 *METALEN: BOT 07 SCANNER*")
    ...
    rapport = f"🥇🥈🥉 *RENDEMENTSRAPPORT BOT 07*\n----------------------------------\n"
